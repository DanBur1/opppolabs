...
enum color {red, orange, yellow, green, blue, purple, pink, white, black, brown, gray, none};
...
public:
    ...
    OfficeSupplies(float p, const string& phone) : price(p), phone_number(phone) {}
...
public:
    ...
    void displayInfo() const override {
        OfficeSupplies::displayInfo();
        cout << " Бумага | Плотность: " << density << "г/м2 | Ширина : " << width << "мм | Длина : " << length << "мм"<< endl;
    }
};
...
int main() {
    ...
    case 1:
        ...
        break;
    case 2:
        cout << "Введите название файла (/ConsoleApplication5/command_input.txt - по умолчанию)";
...
void processCommand(string command) {
    ...
    case 1: //add
    {
        ...
        if (parameters[1] == "pencil") {
            ...
            Pencil* sup = new Pencil(param_price, parameters[3], param_density, param_color);
            ...
            if (parameters[1] == "pen") {
                ...
                if (parameters[1] == "paper") {
                    ...
                    Paper* sup = new Paper(param_price, parameters[3], param_density, param_width, param_length);
                    supplies.push_back(sup);
                    cout << "Добавлен ";
                    sup->displayInfo();
                }
    ...
    case 2: { //rem
        ...
        cout << "Предмет под номером " << id << " будет удалён." << endl << "Удаление ";
    ...
    case 4: //help
    {
        cout << "Редактор списка канцелярских принадлежностей." << endl
            << "Используются 4 комманды: add (Добавить), rem (Удалить), print (Вывести), exit (Выход)." << endl
            << "Имеется 3 типа принадлежностей: Pencil (Карандаш), Pen (Ручка), Paper (Бумага)." << endl
